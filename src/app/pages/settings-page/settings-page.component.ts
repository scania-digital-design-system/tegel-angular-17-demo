...
export default class SettingsPageComponent implements OnInit, OnDestroy {
  ...
  submitForm(event: SubmitEvent) {
    ...
    this.userStoreService.updatePlaceOfWork(
      this.formField.get("placeOfWork")?.value
    );
    form.reset();
  }

  ngOnInit() {
    this.userStoreService.userName$
      .pipe(takeUntil(this.onDestroy$))
      .subscribe((userName) => {
        this.userName = userName;
      });
    this.userStoreService.placeOfWork$
      .pipe(takeUntil(this.onDestroy$))
      .subscribe((placeOfWork) => {
